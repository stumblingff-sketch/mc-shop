...
import json
import os
# Removed 'import random' as it is no longer needed
...
def inject_config():
    ...
    try:
        ...
        return dict(config={
            ...
            'logo_url': 'https://static.wikia.nocookie.net/minecraft_gamepedia/images/6/6e/Red_Apple_JE2_BE2.png',
            'hero_background': 'https://i.ibb.co/bM3tjBM3/8d3783658812bb071591febfca65c60b609fdfa0headerbg.webp'
        })
...
def home():
    ...
    # 1. Get Real Featured Items (Top 3 most expensive Ranks)
    featured = Rank.query.filter_by(category='rank').order_by(Rank.price.desc()).limit(3).all()
    
    # 2. Recent Payments (Cleaned - Empty for now)
    # Later we will connect this to a real Payment database
    recent_payments = [] 
...
def login():
    if request.method == 'POST':
        ...
        if user and check_password_hash(user.password, password):
            login_user(user)
            if user.id == 1: return redirect(url_for('admin_panel'))
...
def add_to_cart(id):
    ...
    cart.append(item.id)
    session['cart'] = cart
    flash(f'"{item.name}" was added to your cart!', 'success')
